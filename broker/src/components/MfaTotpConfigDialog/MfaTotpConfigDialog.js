...
function MfaTotpConfigDialog(props) {
		...
		Auth.currentAuthenticatedUser()
			.then((CognitoUser) => {
				Auth.setupTOTP(CognitoUser)
					.then((code) => {
						setQrUrl(
							`otpauth://totp/AWSCognito:${CognitoUser.username}?secret=${code}&issuer=${Branding.appName}`
						);
						setActiveStep(1);
					})
					.catch((err) => {
						...
						setSnackBarOps({
							...
							autoHide: 3000,
							message: I18n.get(
								"MFA_TOTP_CONFIG_DIALOG_MESSAGE_EROR"
							),
						});
					});
			})
	...
	return (
		<div className={classes.root}>
			...
			>
				<DialogTitle
					id="mfa-totp-configure-dialog-title"
						...
						<Step key="step1">
							...
							<StepContent>
								...
								<div className={classes.actionsContainer}>
									...
									>
										{I18n.get(
											"MFA_TOTP_CONFIG_DIALOG_BUTTON_NEXT_LABEL"
										)}
									</Button>
									<Button
									...
									>
										{I18n.get(
											"MFA_TOTP_CONFIG_DIALOG_BUTTON_BACK_LABEL"
										)}
									</Button>
								</div>
							</StepContent>
						...
						<Step key="step2">
							...
							<StepContent>
								...
								<div className={classes.actionsContainer}>
									...
									>
										{I18n.get(
											"MFA_TOTP_CONFIG_DIALOG_STEP3_BUTTON_ACTIVATE_TOTP_LABEL"
										)}
									</Button>
									<Button
										disabled={activeStep === 0}
